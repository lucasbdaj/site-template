...
export default function Contato({ config }: { config: ClientConfig }) {
  ...
  const horariosEstruturados = horario.length > 0 && typeof horario[0] === "object";
        ...
        >
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 14, marginBottom: 24, alignItems: "flex-start" }}>
              ...
              <div>
                <p style={{ fontSize: 11, opacity: 0.7, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Endereço</p>
            ...
            {!config.whatsapp && config.telefone && (
              <div style={{ display: "flex", gap: 14, marginBottom: 24, alignItems: "flex-start" }}>
                ...
                <div>
                  <p style={{ fontSize: 11, opacity: 0.7, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Telefone</p>
                  <a href={ctaHref} style={{ fontSize: 14, color: "white", textDecoration: "none" }}>
            ...
            {config.email && (
              <div style={{ display: "flex", gap: 14, marginBottom: 24, alignItems: "flex-start" }}>
                ...
                <div>
                  <p style={{ fontSize: 11, opacity: 0.7, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>E-mail</p>
            ...
            {horario.length > 0 && (
              <div style={{
                ...
                <p style={{ fontSize: 11, opacity: 0.7, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 14 }}>
                  Horário de Atendimento
                </p>
                {horariosEstruturados ? (
                  horario.map((h, i) => {
                    const item = h as { dia: string; horario: string };
                    return (
                      <div key={i} style={{
                        display: "flex",
                        ...
                        alignItems: "center",
                        padding: "7px 0",
                        borderBottom: i < horario.length - 1 ? "1px solid rgba(255,255,255,0.08)" : "none",
                      }}>
                        <span style={{ fontSize: 13, opacity: 0.85 }}>{item.dia}</span>
                        <span style={{
                          fontSize: 13,
                          fontWeight: 600,
                          color: item.horario === "Fechado" ? "rgba(255,255,255,0.4)" : "white",
                        }}>{item.horario}</span>
                      </div>
                    );
                  })
                  ...
                  horario.map((h, i) => (
                    <p key={i} style={{ fontSize: 13, opacity: 0.85, marginBottom: 6 }}>{h as string}</p>
                  ))
                )}
              </div>
            )}
