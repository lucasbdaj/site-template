...
export default function ContatoBasisDatum({ config }: { config: ClientConfig }) {
  ...
  const iconChip = (bg: string, children: ReactNode) => (
    <div style={{
            ...
            {horario.length > 0 && (
              <div style={{ marginTop: 6, borderTop: "1px solid rgba(255,255,255,0.14)", paddingTop: 20 }}>
                ...
                {horariosEstruturados ? (
                  horario.map((h, i) => {
                    ...
                    return (
                      <div key={i} style={{
                        ...
                        <span style={{ fontSize: 13, opacity: 0.8 }}>{item.dia}</span>
                        <span style={{ fontSize: 13, fontWeight: 600, color: item.horario === "Fechado" ? "rgba(255,255,255,0.4)" : "white" }}>
                          {item.horario}
                        </span>
                      </div>
                    );
                  ...
                  horario.map((h, i) => (
                    <p key={i} style={{ fontSize: 13, opacity: 0.8, marginBottom: 6 }}>{h as string}</p>
                  ))
                )}
              </div>
            )}
