...
const estiloLabel = { display: "block", fontSize: 13, fontWeight: 500, color: "#1a2e4a", marginBottom: 6 };
const estiloInput = {
  width: "100%", padding: "10px 14px", borderRadius: 8,
  border: "1px solid #e5e7eb", fontSize: 14, fontFamily: "inherit",
};
const estiloCard = { background: "#ffffff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 32, marginBottom: 20 };
const estiloTituloSecao = { fontSize: 17, fontWeight: 700, color: "#1a2e4a", marginBottom: 20 };
...
export default function FormOrcamento() {
  ...
  function atualizarHorario(i, campo, valor) {
    setHorario(h => h.map((row, idx) => (idx === i ? { ...row, [campo]: valor } : row)));
  }
  function removerLinhaHorario(i) {
  ...
  const input = (label, name, tipo = "text", obrigatorio = false, placeholder = "") => (
    <div>
